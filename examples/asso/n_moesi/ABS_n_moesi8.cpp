/******************************
  Program "../examples/asso/n_moesi/ABS_n_moesi8.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "../examples/asso/n_moesi/ABS_n_moesi8"
...
bool mu__boolexpr2;
bool mu__boolexpr3;
  if (!((mu_a[mu_j]) != (mu_M))) mu__boolexpr3 = FALSE ;
  else {
  mu__boolexpr3 = ((mu_a[mu_j]) != (mu_E)) ; 
}
  if (!(mu__boolexpr3)) mu__boolexpr2 = FALSE ;
  else {
  mu__boolexpr2 = ((mu_a[mu_j]) != (mu_OSTATUS)) ; 
}
if ( !(mu__boolexpr2) )
  { mu__quant1 = FALSE; break; }
};
};
    return mu__quant1;
  }
...
bool mu__boolexpr5;
bool mu__boolexpr6;
  if (!((mu_a[mu_j]) != (mu_M))) mu__boolexpr6 = FALSE ;
  else {
  mu__boolexpr6 = ((mu_a[mu_j]) != (mu_E)) ; 
}
  if (!(mu__boolexpr6)) mu__boolexpr5 = FALSE ;
  else {
  mu__boolexpr5 = ((mu_a[mu_j]) != (mu_OSTATUS)) ; 
}
if ( !(mu__boolexpr5) )
...
for(int mu_j = 1; mu_j <= 4; mu_j++) {
bool mu__boolexpr8;
  if (!((mu_a[mu_j]) != (mu_M))) mu__boolexpr8 = FALSE ;
  else {
  mu__boolexpr8 = ((mu_a[mu_j]) != (mu_E)) ; 
}
if ( !(mu__boolexpr8) )
...
for(int mu_j = 1; mu_j <= 4; mu_j++) {
bool mu__boolexpr10;
  if (!((mu_a[mu_j]) != (mu_M))) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = ((mu_a[mu_j]) != (mu_E)) ; 
}
if ( !(mu__boolexpr10) )
...
bool Condition(unsigned r)
{
  ...
  if (r>=4 && r<=7) return R4.Condition(r-4);
  if (r>=8 && r<=11) return R5.Condition(r-8);
  if (r>=12 && r<=15) return R6.Condition(r-12);
  if (r>=16 && r<=19) return R7.Condition(r-16);
...
void Code(unsigned r)
{
  ...
  if (r>=4 && r<=7) { R4.Code(r-4); return; } 
  if (r>=8 && r<=11) { R5.Code(r-8); return; } 
  if (r>=12 && r<=15) { R6.Code(r-12); return; } 
  if (r>=16 && r<=19) { R7.Code(r-16); return; } 
...
int Priority(unsigned short r)
{
  ...
  if (r>=4 && r<=7) { return R4.Priority(); } 
  if (r>=8 && r<=11) { return R5.Priority(); } 
  if (r>=12 && r<=15) { return R6.Priority(); } 
  if (r>=16 && r<=19) { return R7.Priority(); } 
...
char * Name(unsigned r)
{
  ...
  if (r>=4 && r<=7) return R4.Name(r-4);
  if (r>=8 && r<=11) return R5.Name(r-8);
  if (r>=12 && r<=15) return R6.Name(r-12);
  if (r>=16 && r<=19) return R7.Name(r-16);
...
/********************
  Invariant records
 ********************/
int mu__invariant_11() // Invariant "rule_3"
{
bool mu__quant12; 
mu__quant12 = TRUE;
{
...
for(int mu_i = 1; mu_i <= 4; mu_i++) {
bool mu__boolexpr14;
  if (!((mu_j) != (mu_i))) mu__boolexpr14 = TRUE ;
  else {
bool mu__boolexpr15;
  if (!((mu_a[mu_i]) == (mu_OSTATUS))) mu__boolexpr15 = TRUE ;
  else {
  mu__boolexpr15 = ((mu_a[mu_j]) != (mu_OSTATUS)) ; 
}
...
bool mu__condition_16() // Condition for Rule "rule_3"
{
...
int mu__invariant_17() // Invariant "Moesi"
{
bool mu__quant18; 
mu__quant18 = TRUE;
{
...
for(int mu_j = 1; mu_j <= 4; mu_j++) {
bool mu__boolexpr20;
  if (!((mu_i) != (mu_j))) mu__boolexpr20 = TRUE ;
  else {
bool mu__boolexpr21;
  if (!((mu_a[mu_i]) == (mu_M))) mu__boolexpr21 = TRUE ;
  else {
  mu__boolexpr21 = ((mu_a[mu_j]) != (mu_M)) ; 
}
...
bool mu__condition_22() // Condition for Rule "Moesi"
{
...
{"Moesi", &mu__condition_22, NULL, },
{"rule_3", &mu__condition_16, NULL, },
};
