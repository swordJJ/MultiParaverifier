...
module InvLib = struct
  ...
  (*let add inv =
    ...
    | None ->
      ...
      cinv*)
	let add inv=
		(*match List.find (!pairs) 
			~f:(fun (p, _) -> 
							match can_imply p inv ~symIndex:true with
						 |Some(_)->true |None->false ) with*)
		match any_can_be_implied_by inv  ~symIndex:true  with

		|None ->let invSym=andList (form2AllSymForm ~f:(inv) ~types:(!type_defs)) in
			let cinv = form_2_concreate_prop ~id:(!index) inv in
   	   incr index;
...
module Choose = struct
  ...
  let check_level ?(must_new=false) inv =
    ...
    if is_tautology (neg inv) then
      ...
      try
    	...
    	(*		if not (FalseInvLib.mem f) then*)
    				begin
          	if must_new || Smv.is_inv (ToStr.Smv.form_act f) then
           	 new_inv inv
          	else begin
          		(*let tmp=FalseInvLib.add inv in*)
           	 not_inv
          	end
          	end
       (*   else not_inv*)
  ...
  let choose guards assigns cons =
	...
	let chkImpliedOrNew must_new inv=
		let implied_by_old = InvLib.any_can_be_implied_by inv ~symIndex:(!symmetry_index_switch) in
        match implied_by_old with
        | Some(old) -> implied old
