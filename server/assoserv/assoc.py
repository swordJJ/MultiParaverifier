...
class DataProcess(object):
    ...
    def collect_dataset(self):
        ...
        if not os.path.exists('%s/%s.csv' % (self.name,self.name.split('/')[-1])) and not os.path.exists('%s/%s.txt' % (self.name,self.name.split('/')[-1])):
            raise FileExistsError
        if not os.path.exists('%s/%s.csv' % (self.name,self.name.split('/')[-1])) or not self.is_rs_match_dataset():
            return self.txt2csv()
        else:
            return self.readcsv()
    ...
    def is_rs_match_dataset(self):
        ...
        csv_cnt = subprocess.check_output(['wc', '-l', '%s/%s.csv' % (self.name, self.name.split('/')[-1])]).decode('utf-8')
        tail_rs = subprocess.check_output(['tail', '%s/%s.txt' % (self.name, self.name.split('/')[-1])]).decode('utf-8')
        return int(re.findall(r'\d+', csv_cnt)[0]) == int(
            re.findall(r'State Space Explored:.*?(\d+)\sstates', tail_rs, re.S)[0]) + 1

    def readcsv(self):
        print('call read_csv')
        df = pd.read_csv('%s/%s.csv' % (self.name,self.name.split('/')[-1]))
        stateDict = {}
    ...
    def txt2csv(self):
        print('txt2csv...')
        csv_filename = '%s/%s.csv' % (self.name,self.name.split('/')[-1])

        reachset = open('%s/%s.csv' % (self.name,self.name.split('/')[-1])).read()
...
class SlctInv(object):
    ...
    def select_invariant(self, pattern=re.compile(r'P\d'), num_core=multiprocessing.cpu_count(), original_file=None):
        ...
        original_file = "%s/%s.m" % (self.name,self.name.split('/')[-1]) if original_file is None else original_file
    ...
    def run_murphi(self, file, aux_para=''):
        process1 = subprocess.Popen("{0}/src/mu {1}".format(self.murphi_dir, file), shell=True,
        ...
        if not re.search(r'Code generated', stdout.decode('utf-8')):
            ...
            raise ValueError
        print('111','/'.join(file.split('/')[:-1]))
        command2 = "g++ -ggdb -o {0}.o {0}.cpp -I {1}/include -lm".format('/'.join(file.split('/')[:-1]), self.murphi_dir)
        ...
        process = subprocess.Popen("{0}.o -k10000000 {1}".format('/'.join(file.split('/')[:-1]), aux_para), shell=True,
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE)
        ...
        # os.remove('%s.cpp' % ('/'.join(file.split('/')[:-1])))
        # os.remove('%s.o' %  ('/'.join(file.split('/')[:-1])))
    ...
    def parallel(self, index_range, id, translate_dic, original_file):
        ...
        counters = []
        new_file = '%s/%s%s.m' % ('/'.join(original_file.split('/')[:-1]),original_file.split('/')[-1].split('.')[0], id)
        print(new_file)
...
def check_file(name):
    ...
    """
    prefix = name.split('/')[-1]
    print(os.path.exists('%s/%s.m' % (name,prefix)),prefix)
    if os.path.exists('%s/%s.m' % (name,prefix)):
        # Need collect reachable set
        if not os.path.exists('%s/%s.txt' % (name,prefix)) and not os.path.exists('%s/%s.csv' % (name,prefix)):
            print('calling Murphi to generate reachable set...')
            inv_slcter = SlctInv(name, [], set(), home=False)
            inv_slcter.run_murphi('%s/%s.m' % (name,prefix), '-ta >%s/%s.txt' % (name,prefix))
...
if __name__ == "__main__":
    try:
        ...
        if not protocol_name or not (os.path.exists(protocol_name)):
