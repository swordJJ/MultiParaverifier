...
class Refiner(object):
    def __init__(self, name, ins_inv):
        self.PN = name.split('/')[-1]
        self.ins_inv = ins_inv
        self.name = name

    def refine(self, node_num):
        if node_num == 2:
            refiner = Refine_2_NODE(self.name, self.ins_inv)
            refiner.analyzer()
...
class Refine_2_NODE(object):
    def __init__(self, name, ins_inv):
        ...
        self.content = open("%s/%s.m" % (self.name,self.pn)).read()
        self.lemma = set()
        self.rules = self.read_rule(ins_inv)

    def create_abs(self):
        fabs = '%s/ABS_%s.m' % (self.name,self.pn)
        copyfile('%s/%s.m' % (self.name,self.pn), fabs)
        return fabs
    ...
    def write_lemma(self):
        fn_lemma_used = '/'.join(self.name.split('/')[:-1]) + '/'+ self.pn + '/lemma_used.txt'
